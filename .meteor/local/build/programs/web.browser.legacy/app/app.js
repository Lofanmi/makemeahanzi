...
var kIdPrefix = 'make-me-a-hanzi';
var kWidth = 128;

var distance2 = function (point1, point2) {
  var diff = [point1[0] - point2[0], point1[1] - point2[1]];
  return diff[0] * diff[0] + diff[1] * diff[1];
};

var getMedianLength = function (median) {
  var result = 0;
...
var getMedianPath = function (median) {
  var result = [];

  for (var _iterator = _createForOfIteratorHelperLoose(median), _step; !(_step = _iterator()).done;) {
...
var getAnimationData = function (strokes, medians, options) {
  options = options || {};
  var delay = 1024 * (options.delay || 0.3);
  var speed = 1024 * (options.speed || 0.02);
  var lengths = medians.map(function (x) {
    return getMedianLength(x) + kWidth;
  }).map(Math.round);
  var paths = medians.map(getMedianPath);
  var animations = [];
  var total_duration = 0;

  for (var i = 0; i < strokes.length; i++) {
    var offset = lengths[i] + kWidth;
    var duration = (delay + offset) / speed / 60;
    var fraction = Math.round(100 * offset / (delay + offset));
    animations.push({
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                       //
var NEURAL_NET_TRAINED_FOR_STROKE_EXTRACTION;
module.link("/lib/net", {
  NEURAL_NET_TRAINED_FOR_STROKE_EXTRACTION: function (v) {
...
Meteor.startup(function () {
  ...
  var trainedClassifier = function (features) {
    input.w = features;
    var softmax = net.forward(input).w;
    return softmax[1] - softmax[0];
  };

  stroke_extractor.combinedClassifier = function (features) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                       //
module.export({
  NEURAL_NET_TRAINED_FOR_STROKE_EXTRACTION: function () {
    return NEURAL_NET_TRAINED_FOR_STROKE_EXTRACTION;
  }
});
var NEURAL_NET_TRAINED_FOR_STROKE_EXTRACTION = {
  "layers": [{
...
var AbstractStage = /*#__PURE__*/function () {
  ...
  var _proto = AbstractStage.prototype;
  ...
  _proto.validate = function () {
    function validate() {
      ...
      assert(log && log.filter(function (x) {
        return x.cls === 'error';
      }).length === 0);
    }
...
module.link("/lib/animation", {
  getAnimationData: function (v) {
    getAnimationData = v;
  }
}, 1);

var VerifiedStage = /*#__PURE__*/function (_AbstractStage) {
  _inheritsLoose(VerifiedStage, _AbstractStage);

  function VerifiedStage(glyph) {
    var _this;

    _this = _AbstractStage.call(this, 'verified') || this;
    var strokes = glyph.stages.order.map(function (x) {
      return glyph.stages.strokes.corrected[x.stroke];
    });
    var medians = glyph.stages.order.map(function (x) {
      return x.median;
    });
    _this.data = getAnimationData(strokes, medians);
    return _this;
  }

  var _proto = VerifiedStage.prototype;
...
Template.verified_stage.helpers({
  data: function () {
    return Session.get('stages.verified.data');
  }
});
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                       //
Handlebars.registerHelper('selected', function (current, value) {
  return {
    value: value,
...
Handlebars.registerHelper('editable', function (field, value) {
  return "<div class=\"value\" contenteditable=\"true\" " + ("data-field=\"" + field + "\">" + value + "</div>");
});
...
var parseIntWithValidation = function (text) {
  var result = parseInt(text, 10);
  assert(!Number.isNaN(result));
  return result;
};

var parsePinyin = function (text) {
  var pinyin = text.split(',').map(function (x) {
    return x.trim();
  }).filter(function (x) {
    return x.length;
  });
...
Template.metadata.events({
  'blur .value': function (event) {
    var text = $(event.target).text();
    var value = text && text !== unknown ? text : null;
    ...
    var glyph = Session.get('editor.glyph');
    var defaults = cjklib.getCharacterData(glyph.character);
...
Template.metadata.helpers({
  ...
  items: function () {
    ...
    if (!glyph) return;
    var defaults = cjklib.getCharacterData(glyph.character);
    var fields = ['definition', 'pinyin', 'strokes'];
    var result = fields.map(function (x) {
      return {
        field: x,
    ...
    if (cjklib.radicals.radical_to_index_map.hasOwnProperty(glyph.character)) {
      var index = cjklib.radicals.radical_to_index_map[glyph.character];
      var primary = cjklib.radicals.primary_radical[index];
      result[0].separator = '; ';
  ...
  references: function () {
    ...
    return [{
      ...
      href: 'http://www.mdbg.net/chindict/chindict.php?page=worddict' + ("&wdrst=0&wdqb=" + character),
      label: 'MDBG'
    }, {
...
Meteor.startup(function () {
  return Meteor.subscribe('getProgress');
});
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
